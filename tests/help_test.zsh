  ...
  for tool in mkcd cpdir "compozsh --sudo-touch-id" g external-device xcode compozsh; do
    ...
    case $tool in
      ...
      (g) facts=('repository root' 'HEAD' 'staged' 'untracked'
        ...
        'last completed check' 'last snapshot' 'ZSH_GIT_REVIEW_AUTO_REFRESH=0'
        'one repaint' 'update ready' 'No focus' 'private pipe' 'worker/provider pair'
        '30 seconds' 'pause' 'manual success or failure'
        'no results' 'first match' 'one reading level'
        ...
        'staged' 'unstaged' 'first parent' '256 KiB' 'filters' 'containing ='
        'fails closed' 'hunks'
        '800 ms child'
      ...
      (xcode) facts=('nearest' 'workspace' 'project' 'scheme' 'destination'
        ...
        'Following, Paused, Closed, or Unavailable'
        'Narrow terminals' 'full-width' 'fuzzy search' 'Follow latest'
        'device console' 'Errors and faults' 'Exclude contains'
        'Scroll up pauses' 'compact metadata' 'original source lines'
        'Copy filtered logs' 'complete captured matching lines' 'Escape returns to Run'
        'unverifiable identity' 'writer gaps' 'Simulator data/tmp' 'Device Hub'
        'window-opening failure' 'picker-header palette' 'settings and descriptions'
